...
const Analytics = () => {
    ...
    useEffect(() => {
        ...
        axiosBase
            .post(
                "/get-form-analytics",
                { form_id: formId },
                { cancelToken: getFormAnalyticsRef.current?.token }
            )
            .then((res) => {
                setFormAnalytics(res.data.formAnalytics);
                const totalSub =
                    res.data.formAnalytics.sinceCreated.datas.reduce(function (
                        accumulator,
                        currentValue
                    ) {
                ...
                setTodaysSubmissions(
                    res.data.formAnalytics["today"].submissions
                );
            })
            .catch((err) => {
    ...
    const handleFilter = (value) => {
        ...
        if (value !== 999) setGraphFilter(`Last ${value} days`);
        else setGraphFilter("Since created");
    };

    const data = {
        labels:
            selectedFilter !== 999
                ? formAnalytics[`${selectedFilter}days`]?.labels
                : formAnalytics.sinceCreated.labels,
        datasets: [
            {
                data:
                    selectedFilter !== 999
                        ? formAnalytics[`${selectedFilter}days`]?.datas
                        : formAnalytics.sinceCreated.datas,
        ...
        <div>
            <div>
                ...
                >
                    ...
                    <MenuItem
                        selected={selectedFilter === 7}
                        onClick={() => handleFilter(7)}
                    >
                        Last 7 days
                    </MenuItem>
                    <MenuItem
                        selected={selectedFilter === 28}
                        onClick={() => handleFilter(28)}
                    >
                        Last 28 days
                    </MenuItem>
                    <MenuItem
                        selected={selectedFilter === 999}
                        onClick={() => handleFilter(999)}
                    >
                        Since created
                    </MenuItem>
                </Menu>
