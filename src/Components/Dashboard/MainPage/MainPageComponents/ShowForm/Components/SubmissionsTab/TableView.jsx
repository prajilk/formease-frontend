...
export default function TableView({ formData, setFormData }) {
    ...
    const deleteSubmission = (id) => {
        setLoading(true);
        const idToDelete = id;
        const index = formData.form_data.findIndex(
            (item) => item._id_ === idToDelete
        );

        axios
            .post("/delete-this-submission", {
            ...
            .then((res) => {
                ...
                if (res.data.submissionDeleted) {
                    if (index !== -1) {
                        const updatedFormData = formData.form_data.filter(
                            (item) => item._id_ !== idToDelete
                        );
