import { Box, Button, Card, CardContent, Chip, Container, Link, Typography } from '@mui/material';
...
const Register = () => {
    ...
    const register = (e) => {
        e.preventDefault();
        setLoading(true)
        axios.post('/register', { email, fullname, password, country })
            .then(() => {
                setLoading(false)
            ...
            .catch((err) => {
                ...
                if (err.response.status === 409) {
                    ...
                    setRegisterErrorLabel("An account already exists with this email.")
                } else {
    ...
    return (
        <section style={{ display: 'flex', justifyContent: 'center', backgroundColor: '#f7fafc' }}>
                ...
                <Box sx={{ width: '100%', mt: 3 }}>
                    ...
                    <form onSubmit={register}>
                        <Card sx={{ boxShadow: '0 30px 60px -12px rgba(50,50,93,0.25),0 18px 36px -18px rgba(0,0,0,0.3)', borderRadius: '10px' }}>
                            <CardContent sx={{ p: 5 }}>
                                <Typography sx={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: '1.3rem', mb: 2 }}>Create an account</Typography>
                                {isRegisterError && <Chip
                                    avatar={<Error style={{ color: 'rgb(237,95,116)', width: '1rem' }} />}
                                    label={registerErrorLabel}
                                    variant="outlined"
                                    color='error'
                                    sx={{
                                        border: 'none', mb: 1, '& > span': {
                                            whiteSpace: 'normal'
                                        }
                                    }} />}
                                <Box>
                                ...
                                <Box>
                                    ...
                                    <select name="country" id='country' required onChange={(e) => setCountry(e.target.value)} defaultValue="">
                    ...
                    <Box>
                        <Typography align='center' sx={{ fontFamily: 'Poppins', py: 5 }}>
                            &copy; formease · <Link href='/contact' sx={{ textDecoration: 'none', color: 'inherit', ':hover': { color: '#00bfff' } }}>Contact</Link>
                        </Typography>
                    </Box>
                </Box>
            </Container>
