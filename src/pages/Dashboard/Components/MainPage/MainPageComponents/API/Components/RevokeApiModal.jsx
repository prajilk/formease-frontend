...
const RevokeApiModal = ({ revokeApiOpen, setApiRevoked, setRevokeApiOpen, setApiKey }) => {
    ...
    const handleRevokeKey = () => {
        ...
        axiosBase.get('/revoke-api', { cancelToken: revokeApiRef.current?.token }).then((res) => {
    ...
    return (
        <Modal
            ...
            open={revokeApiOpen}
            onClose={() => { setRevokeApiOpen(false); revokeApiRef.current?.cancel() }}
                ...
                <Card sx={revokeApiSx}>
                    <CardContent sx={{ p: 0 }}>
                        <Box sx={{ px: 2, py: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <IconButton onClick={() => setRevokeApiOpen(false)} style={{ padding: 1 }}>
                                <CloseRounded />
                            </IconButton>
                        </Box>
                    ...
                    <CardActions sx={{ justifyContent: 'flex-end' }}>
                        <Button
                            ...
                            sx={{ width: '100%' }}
                            disabled={loading}>
                            {loading ? <div className='loadingRed'></div> : 'Revoke API Key'}
                        </Button>
                    </CardActions>
                </Card>
            </Fade>
